...
| 真机旅行 E2E | `testLiveTravelTaskEndToEndOnDevice` + 手工商务出差测试包 | 真实 Qwen、Gmail/Drive、日历读写、MapKit 地点与路线、相册 OCR、双 PDF、完成通知 | 手动显式开启 |
...
- Apple 日历中的定时忙碌事件必须参与商务出差冲突检查；全天节假日只能作为上下文，不能阻塞整天或导致 Agent 耗尽轮次。
- 每个商务出差交通项目都必须引用手机端 MapKit 路线；起终点、出发时刻和最低交通时长不得由模型改写。
...
- 存在用户选择的票据时，Drive 必须用不同幂等键上传行程 PDF 和报销 PDF，并在聊天中返回两个链接；重试不得覆盖另一份文件。
- 相册图片 OCR 只能处理用户明确选择的图片；报销数据不得脱离可见证据生成，重复发票号/订单号必须确定性去重，缺失凭证号与超出行程日期必须标记待核对。
...
- 航班日历事件应同时包含提前 24 小时的值机提醒和提前 3 小时的出发提醒；交通事件使用日历提醒而不是提醒事项。
